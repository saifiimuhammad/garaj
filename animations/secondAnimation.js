...
function resetSecondAnimation() {
  ...
  if (secondIntroTimeline) { secondIntroTimeline.kill(); secondIntroTimeline = null; }
...
function buildLPathForBox(boxEl) {
  ...
  const sibTopRect    = (isLeft ? qs("secondBoxLeftTop")    : qs("secondBoxRightTop")).getBoundingClientRect();
  const sibBottomRect = (isLeft ? qs("secondBoxLeftBottom") : qs("secondBoxRightBottom")).getBoundingClientRect();
  ...
  const startY = isTop ? (boxRect.bottom - rootRect.top) : (boxRect.top - rootRect.top);
...
function runSecondIntro() {
  ...
  "revealPhone"
    ...
    .to(phoneRect,{
      ...
      duration:0.5,
      ease:"power1.out",
      onComplete: () => { phoneRect.style.visibility = "hidden"; }
    }, ">-0.1");
}
